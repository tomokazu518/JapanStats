...
# 行政区域を都道府県に集約
japan <- japan %>% mutate(code = as.numeric(substr(N03_007, 1, 2)))
japan_prefectures <- aggregate(japan, list(japan$code), head, n = 1) # けっこう時間がかかる
...
st_write(japan_prefectures, "output/japan_prefectures.geojson")
...
ggsave("output/map_japan.pdf", device = cairo_pdf)

# 沖縄県をインセットに入れる
japan_prefectures$geometry[[47]] = japan_prefectures$geometry[[47]] + c(7,15)

japan_prefectures %>% 
  ggplot() +
  geom_sf() +
  geom_segment(x=128, y=38, xend=133, yend=38)+
  geom_segment(x=133, y=38, xend=137, yend=40)+
  geom_segment(x=137, y=40, xend=137, yend=44)
